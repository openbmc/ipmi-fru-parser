...
int parse_fru_area (const uint8_t area, const void* msgbuf, const size_t len, fruInfo& info)
{
  int rv = -1;
  int i = 0;
...
#endif
                info[i] = make_pair(vpd_key_names[i],std::to_string(chassis_type));
                continue;
            }
            info[i] = make_pair(vpd_key_names[i],std::string((char*)(vpd_info[i].type_length_field)));
...
#endif
                    info[i] = make_pair(vpd_key_names[i],std::string(timestr));
                    continue;
                }
                info[i] = make_pair(vpd_key_names[i],std::string((char*)vpd_info[i].type_length_field));
