...
// Volts to 10mV units
static constexpr int Vto10mV(const double V)
{
...
constexpr uint16_t mrBits(const uint16_t r, const IpmiMultirecordParam& p)
{
    struct MaskShift
    {
    ...
    uint16_t mask = (1 << ms[index].nbits) - 1;
...
{
  public:
    PowerSupplyClass() = delete;
    explicit PowerSupplyClass(const uint8_t* data)
    ...
    virtual ipmi::vpd::PropertyMap updatePropertyMap() const final
    {
        ...
        props.emplace("HoldUpTime", static_cast<int64_t>(holdUpTime()));
        props.emplace("PeakCapacity",
                      static_cast<int64_t>(peakCapacity()));
        ...
        if (predFailSupport())
        {
            props.emplace("TachoUses2PulsesPerRotation", tachoPPR());
        }
        else
        {
...
{
  public:
    DcOutputClass() = delete;
    explicit DcOutputClass(const uint8_t* data) :
        pData(*reinterpret_cast<const DcOutputInfo*>(data))
    {
        // Perform the IPMI-to-host endianness conversion for
        // multi-byte data fields:
                        ...
                        const size_t dataLen)
{
    ...
    switch (recordType)
    {
        case IpmiMultirecordType::powerSupplyInfo:
            expectedLength = sizeof(PowerSupplyInfo);
            break;
        case IpmiMultirecordType::dcOutput:
            expectedLength = sizeof(DcOutputInfo);
            break;
        default:
    ...
    if (expectedLength)
    {
        ...
        else
        {
            ...
            {
                case IpmiMultirecordType::powerSupplyInfo:
                    record = unique<PowerSupplyClass>(recordData);
                    break;
                case IpmiMultirecordType::dcOutput:
                    record = unique<DcOutputClass>(recordData);
                    break;
                default:
        ...
        if (record)
        {
            ...
            switch (record.type)
            {
                ...
                case IpmiMultirecordType::dcOutput:
                    ...
                    log<level::DEBUG>("Record DcOutputInfo",
