                           ...
                           const size_t dataLen)
{
    ...
    {
        log<level::ERR>("Data shorter than header lenght");
        return NULL;
    }
    ...
    // parse header
    const MultirecordHeader* header =
        reinterpret_cast<const MultirecordHeader*>(recordData);

    // set type
    IpmiMultirecordType recordType =
        static_cast<IpmiMultirecordType>(header->recordTypeId);

    if (recordType > MAX_PARSE_TYPE)
    {
