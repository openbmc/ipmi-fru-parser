...
auto print = [](InterfaceList&& object, const std::string& path)
{
    std::cout << "\n";
    std::cout << path << "\n";
    std::cout << "\n";
...
int ipmi_update_inventory(fru_area_vec_t& area_vec)
{
    ...
    for (const auto& instance : instanceList)
    {
        ...
        for (const auto& interfaceList : instance.second)
        {
            ...
            for (const auto& properties : interfaceList.second)
            {
                ...
                for (const auto& info : properties.second)
                {
                    if (info.first == "FRUSection")
                    {
                        section = info.second;
                    }
                    if (info.first == "IPMIPropertyName")
                    {
                ...
                prop.emplace(properties.first, value);
            }
            interfaces.emplace(interfaceList.first, prop);
        }
        //TODO:- remove it later with the inventory manager call.
        print(std::move(interfaces), instance.first);
    }
