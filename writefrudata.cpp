                ...
                const std::string& path)
{
    ...
    {
        lg2::error("Exception from sdbus call: {EC}", "EC", ex);
        throw;
    }
...
int updateInventory(FruAreaVector& areaVector, sdbusplus::bus_t& bus)
{
    ...
    for (const auto& fruArea : areaVector)
    {
        ...
        if (rc < 0)
        {
            lg2::error("Error parsing FRU records");
            return rc;
        }
    ...
    {
        lg2::error("Failed to get service: {EC}", "EC", e);
        return -1;
    }
    ...
    {
        lg2::error(
            "Error in notify call, service: {SERVICE}, path: {PATH}, error: {EC}",
            "SERVICE", service, "PATH", path, "EC", ex);
        return -1;
    }
